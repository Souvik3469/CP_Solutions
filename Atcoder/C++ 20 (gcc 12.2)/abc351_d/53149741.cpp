...
void solve()
{
     ll n, m;
        ...
        unordered_map<ll, ll> good;
        DSU d(n * m + 1);
        vector<set<int>> st(n * m + 1);
        for (ll i = 0; i < n; i++) {
            for (ll j = 0; j < m; j++) {
                ll flag = 1;
                if (s[i][j] == '.') {
                    ...
                    for (ll di = -1; di <= 1; di++) {
                        for (ll dj = -1; dj <= 1; dj++) {
                            if (abs(di) + abs(dj) == 1 && isvalid(i + di, j + dj, n, m)) {
                                if (s[i + di][j + dj] == '#') {
                                    flag = 1;
                                }
                            }
                    ...
                    for (ll di = -1; di <= 1; di++) {
                        for (ll dj = -1; dj <= 1; dj++) {
                            if (abs(di) + abs(dj) == 1 && isvalid(i + di, j + dj, n, m)) {
                                if (good[
                                    (i + di) * m + j + dj]) {
                                    d.Union(i * m + j, (i + di) * m + j + dj);
                                } else if (s[i + di][j + dj] == '.') {
                                    st[i * m + j].insert((i + di) * m + j + dj);
                                }
                            }
                        }
                    }
        ...
        for (ll i = 0; i < n; i++) {
            for (ll j = 0; j < m; j++) {
                if (good[i * m + j]) {
                    int p = d.find(i * m + j);
                    st[p].insert(st[i * m + j].begin(), st[i * m + j].end());
                    inc(mp, p);
                }
            }
        }
