...
void inc(unordered_map<ll, ll>& mp, ll key) {
...
void solve()
{
     ll n, m;
        ...
        DSU d(n * m + 1);
        vector<vector<ll>>type3(n,vector<ll>(m));
        vector<set<ll>>type2(n*m+1);
        ll dx[4]={-1,0,+1,0};
        ll dy[4]={0,+1,0,-1};
        for(ll i=0;i<n;i++){
            for(ll j=0;j<m;j++){
              ll  fl=1;
                if(s[i][j]=='.'){
                    ...
                    for(ll k=0;k<4;k++){
                        ll ni=i+dx[k];
                        ll nj=j+dy[k];
                        if(isvalid(ni,nj,n,m) and s[ni][nj]=='#'){
                            fl=1;
                        }
                    }
                }
        ...
        unordered_map<ll,ll>mp;
          for(ll i=0;i<n;i++){
            for(ll j=0;j<m;j++){
              if(type3[i][j]==1){
                  ll par=d.find(i*m+j);
                 type2[par].insert(type2[i*m+j].begin(),type2[i*m+j].end());
                  inc(mp,par);
              }
            }
        }
        ...
        // for(ll i=0;i<z;i++){
        //     debug(i);
        //     for(auto y:type2[i])cout<<y<<" ";
        //     cout<<endl;
        // }
        for(auto x:mp){
            //debug(x.ff);
            // debug(x.ss);
            //debug(type2[x.ff].size());
            mp[x.ff]+=type2[x.ff].size();
           // debug(x.ss);
        }
        // for(auto x:mp)cout<<x.ff<<" "<<x.ss<<endl;
        ll mx=0;
