...
vector<ll> LIS(vector<ll> &nums,ll k)
{
 ...
 ll fgame(ll mask,ll t,ll n,vector<vector<ll>>&adj,vector<vector<ll>>&dp){
     ...
     for(auto i:rem){
         for(auto j:adj[i]){
             if(!(mask&(1ll<<j))){
                 if(t==0)ans=ans&(fgame(mask+(1ll<<i)+(1ll<<j),1-t,n,adj,dp));
                 else ans=ans|(fgame(mask+(1ll<<i)+(1ll<<j),1-t,n,adj,dp));
             }
         }
     }
 ...
 static bool cmp1(const pair<pair<ll,ll>,ll>&p1 ,const pair<pair<ll,ll>,ll>&p2){
   ...
   Trie *getnode(){
       Trie* node=new Trie();
       node->c=1;
       for(ll i=0;i<26;i++)node->a[i]=NULL;
       return node;
   }
   void insert(Trie *root,string &s,ll &ans){
       
       for(ll i=0;i<s.size();i++){
           if(root->a[s[i]-'a']==NULL){
               root->a[s[i]-'a']=getnode();
               root=root->a[s[i]-'a'];
           }
           else{
               ans+=root->a[s[i]-'a']->c;
