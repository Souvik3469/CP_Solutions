...
void dfs(ll i,ll &node,vector<vector<ll>>&g,vector<ll>&vis,map<pair<ll,ll>,ll>&mp){
    vis[i]=1;
    node++;
    
    for(auto x:g[i]){
        if(vis[x]==0)dfs(x,node,g,vis,mp);
        
        if(mp.find({i,x})==mp.end() and mp.find({x,i})==mp.end())mp[{x,i}]++;
    }
}
void solve()
{
